            ...
            <table className="mt-6 w-full whitespace-nowrap text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-300">
                {(searchQuery ? allWithdrawalRequest : allwithdrawal)
                  ?.filter((item) => item?.type == "working" && (filterquery ? item?.status === filterquery : true))
                  ?.map((item, index) => (
                        ...
                        {item?.trc20 && (
                          <>
                            {/* <div className="flex gap-2 items-center mt-2 text-[12px]">
                              <span className="block"> {item?.trc20}</span> */}
                              <button onClick={()=>handleScan(item?.trc20, 'trc20')} className="px-3 py-1 bg-indigo-500 hover:bg-indigo-600 text-[12px] rounded-md">
                                Scan
                              </button>
                            {/* </div> */}
                          </>)}
                        ...
                        <div className="rounded bg-gray-700/40 px-2 py-1 text-xs font-medium text-gray-400 ring-1 ring-gray-300">
                          {editMode && editableWithdrawal?.id === item?.id ? (
                            <select
                              id="status"
                              name="status"
                              className="border-0 px-3 py-1 bg-gray-200 rounded-sm text-sm text-gray-600 shadow focus:outline-none w-full"
