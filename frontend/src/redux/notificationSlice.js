...
export const createNotification = createAsyncThunk(
  ...
  async (notificationData, { rejectWithValue }) => {
    try {
      const response = await axios.post(`https://api.jastrobot.com/api/v1/notifications`, notificationData);
...
export const getAllNotifications = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get(`https://api.jastrobot.com/api/v1/notifications`);
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response.data);
    }
...
export const getUserNotifications = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      const response = await axios.get(`https://api.jastrobot.com/api/v1/notifications/user/${userId}`);
...
export const markNotificationAsSeen = createAsyncThunk(
  ...
  async ({ userId, notificationId }, { rejectWithValue }) => {
    try {
      const response = await axios.put(`https://api.jastrobot.com/api/v1/notifications/${notificationId}/seen`, { userId });
...
export const deleteNotification = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await fetch(`https://api.jastrobot.com/api/v1/notifications/delete/${id}`, {
        method: "DELETE",
      });
