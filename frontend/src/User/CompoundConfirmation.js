...
import { compoundAmount, clearErrors, clearMessage } from "../redux/withdrawalSlice";

export default function CompoundConfirmation({ openModel, modelClose}) {
  ...
  const handleSaveChanges = (e) => {
    ...
    if (form.checkValidity()) {
      ...
      dispatch(compoundAmount({ values: allValues }));
      modelClose();
    } else {
      form.reportValidity();
    }
              ...
              <div className="p-5">
                <div className="py-4 flex justify-between items-center">
                  <h2 className="text-xl font-semibold mb-5 text-gray-300 ">
                    Compound Amount form (Your Wallet : {singleuser?.business})
                  </h2>
                ...
                <form>
                  ...
                  <p className="text-gray-200 text-[12px] mt-2">
                    T & B C: The minimum withdrawal amount is $10, and there is an admin charge of 10%. Withdrawals are processed within 72 hours
                  </p>
                  <div className="flex justify-between mt-4">
                    <button
                      type="button"
                      className="w-full p-2 bg-gray-800 hover:bg-gray-900 border text-white rounded mr-2"
                      onClick={handleSaveChanges}
                    >
                       {loading ? <Spinner /> : "compound"}
                    </button>
                    <button
                      type="submit"
